...
struct {
  ...
  SELECT_L_PIN, { WINK_L_PIN, NOBLINK }  //,
...
void drawEye(            // Renders one eye.  Inputs must be pre-clipped & valid.
  ...
  for (screenY = 0; screenY < SCREEN_HEIGHT; screenY++, scleraY++, irisY++) {
    ...
    for (screenX = 0; screenX < SCREEN_WIDTH; screenX++, scleraX++, irisX++) {
      if ((pgm_read_byte(lower + screenY * SCREEN_WIDTH + screenX) <= lT) || (pgm_read_byte(upper + screenY * SCREEN_WIDTH + screenX) <= uT)) {  //Covered by eyelid
        p = 0;
      ...
      *(pbuffer + pixels++) = p >> 8 | p << 8;
      if (pixels >= BUFFER_SIZE) {
        yield();
        tft.pushColors((uint8_t*)pbuffer, pixels * 2);
        pixels = 0;
      }  //drawEye
    }
  }
  if (pixels) {
    tft.pushColors(pbuffer, pixels);
    pixels = 0;
  }
}
...
void frame(                     // Process motion for a single frame of left or right eye
  ...
  if (isSerial) {
    if (Serial.available()) {
      eyeX = (Serial.readStringUntil(',').toFloat() * 1024) / 640;
      eyeY = (Serial.readStringUntil('\n').toFloat() * 1024) / 480;
...
// Blinking
  ...
  iScale = ((IRIS_MAP_HEIGHT + 1) * 1024) / (1024 - (iScale * (IRIS_MAP_HEIGHT - 1) / IRIS_MAP_HEIGHT));
